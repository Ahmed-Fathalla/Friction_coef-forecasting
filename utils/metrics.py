...
import numpy as np
import pandas as pd
from sklearn.metrics import mean_absolute_error as MAE_np,\
                            mean_squared_error as MSE_np,\
                            r2_score as R2_np
...
met_dic = {POCID:POCID_np,
           u_theil:u_theil_np,
           RootMeanSquaredError:RMSE_np
           }

def get_res(y_true, y_pred, round_=7,metric_lst=[]):
    # y_true = np.array(y_true)
    # y_pred = np.array(y_pred)
    results = []
    for metric in metric_lst:
        if isinstance(metric, str):
            if metric=='mae':metric=MAE_np
            elif metric=='mse':metric=MSE_np
            elif metric=='rmse':metric=RMSE_np
            elif metric=='R2':metric=R2_np
